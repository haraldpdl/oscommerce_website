 ...
 */
  ...
  class GetPartnerStatusUpdates {
    public static function execute() {
      ...
      $Qpartners = $OSCOM_PDO->prepare('select p.title, p.url, su.status_update from :table_website_partner p, :table_website_partner_transaction t, :table_website_partner_status_update su where t.package_id = 3 and t.date_start <= now() and t.date_end >= now() and t.partner_id = p.id and p.id = su.partner_id and su.code = :code group by p.id order by rand() limit 5');
